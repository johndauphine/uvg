...
Apply atomicity is per-dialect. On PostgreSQL, each apply batch and each migration section runs inside a single transaction: a failure rolls the batch back and leaves the target unchanged, and statements that cannot run inside a transaction block (`CREATE INDEX CONCURRENTLY`, `VACUUM`, ...) automatically fall back to statement-by-statement apply. On SQL Server, MySQL, and SQLite, DDL implicitly commits, so migration DDL is applied statement-by-statement and earlier statements in a failed migration may already be applied. Either way, on failure UVg stops at the first failed statement and prints the migration revision, section, statement number, dialect error, and failed SQL without printing the target URL — and says whether the section was rolled back. `uvg_version` is advanced only after all `-- PRE`, `-- UP`, and `-- POST` SQL succeeds; downgrades move or clear `uvg_version` only after all down SQL succeeds. See [Operations and Security](docs/operations-security.md) for the full guarantees.
